...
var createJSON = function() {
	...
	for (let i = 0; i < len; i++) {
		...
		// 조리시간을 초로 환산
		let recipe_reqTime = parseInt(hour) * 1800 + parseInt(minute) * 60
				+ parseInt(seconds);

		
		var recipe  = {
			"recipe_processNo" : recipe_processNo.toString(),
			"recipe_action" : recipe_action,
			"recipe_fire" : recipe_fire.toString(),
			"recipe_reqTime" : recipe_reqTime.toString(),
			"recipe_image" : ""
		}
